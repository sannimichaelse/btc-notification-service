...
BITCOIN_PRICE_THRESHOLD = 30000 # you can set this to any value
...
def post_ifttt_webhook(event, value):
    ...
    data = {'value1': bitcoin_value} # The payload that will be sent to IFTTT service
    logging.info(data)
...
def format_bitcoin_history(bitcoin_history):
    ...
    for bitcoin_price in bitcoin_history:
        ...
        currency_string = "{:,.2f}".format(price)
        # <b> (bold) tag creates bolded text
        row = '{}: $<b>{}</b>'.format(date, currency_string)  # 24.02.2018 15:09: $<b>10123.4</b>
        rows.append(row)
...
def main():
    ...
    while True:
        ...
        if len(bitcoin_history) == 2: # Once we have 2 items in our bitcoin_history send an update
